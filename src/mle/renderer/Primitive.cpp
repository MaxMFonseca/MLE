...
Primitive::PbrMaterial loadMaterial(const GLTF& gltf, int material_index) {
    auto& texture_cache = Renderer::i().textureCache();

    Primitive::PbrMaterial out{};
...
void Primitive::load(const GLTF& gltf, usize mesh_idx, usize primitive_idx) {
    const auto& model = gltf.model();
    ...
    if (textured_) {
        ...
        if (skinned_) {
            ...
            for (usize i = 0; i < vert_count; ++i) {
                texture_vertices_skinned.push_back(PbrTextureSkinnedVertex{
                    .pos = positions[i], .normal = normals[i], .uv = uvs[i], .tangent = tangents[i], .joint0 = joints0[i], .weight0 = weights0[i]});
            }
        ...
        for (usize i = 0; i < vert_count; ++i) {
            color_vertices.push_back(
                PbrColorVertex{.pos = positions[i], .normal = normals[i], .color = colors[i], .mre = vec3f{metals[i], roughs[i], emiss[i]}});
        }
